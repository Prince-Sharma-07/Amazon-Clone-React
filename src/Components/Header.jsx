...
export default function Header() {
  ...
  return (
    <div className='w-full fixed z-10 bg-[#131921] flex px-4 text-white text-sm font-amazonEmber items-center max-md:h-12 md:h-15'>

      <div onClick={() => navigate('/home')} className={`${borderStyles} h-[90%] flex items-center md:px-1 md:py-4 lg:px-2 lg:py-6 `}>
        <img src={AmazonLogo} alt="Logo" className=' max-md:h-4 md:h-8 ' />
      ...
      <div className={`${borderStyles} h-[90%] flex items-center p-1`}>
        ...
        <div className='flex flex-col'>
          <span className='text-[8px] sm:text-[10px] md:text-[12px] text-[#e5e5e6] leading-3 max-md:hidden'>Delivering to Saharanpur 247001</span>
      ...
      <div onClick={() => navigate('/cart')} className={`${borderStyles} h-[90%] px-2 flex items-end relative`}>
        <div>
          <span className='fixed right-[68px] top-3 text-[8px] sm:text-[10px] md:text-[12px] lg:text-[14px] font-[700] text-orange-400'>{totalItems}</span>
